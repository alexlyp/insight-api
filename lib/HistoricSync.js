...
HistoricSync.prototype.showProgress = function() {
  ...
  if (self.error)
    ...
    self.updatePercentage();
      info(util.format('%s status: [%d%%]', moment().format(), self.syncPercentage));
  }
    ...
    self.sync.bDb.getTip(function(err, tip, height) {
      ...
      async.doWhilst(
        function(cb) {
	     ...
	     if (!bi)  {
	       self.sync.bDb.getPrev(tip, function(err, val) {
		 self.sync.bDb.fromHashWithInfo(val, function(err, prevBlock) {
	           if (err) return next(err);
		   if (!prevBlock) return cb();
                   
                   self.sync.bDb.setTip(blockInfo.hash, blockInfo.height, cb);
	           return next();
	         });
		 return next(err);
	       });
	    ...
	    else if (blockInfo.height == -1) { 
              // if blockInfo.height == -1 then fork has occurred, reset tip to previousblockhash
              self.sync.bDb.setTip(blockInfo.previousblockhash, height-1, cb);
	      return next();
	    } else
              return cb();
          });
